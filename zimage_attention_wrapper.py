"""
Z-Image IP-Adapter Attention Wrapper

EXACT COPY of SD3's working implementation since Z-Image uses JointTransformerBlock.
This ensures compatibility with ComfyUI's patching system.
"""
...
class ZImageIPAttnProcessor(nn.Module):
    """
    EXACT COPY of SD3's IPAttnProcessor but renamed and dimensioned for Z-Image.
    """

    def __init__(
        self,
        hidden_size=3840,                    # Z-Image dimension (expanded from SD3's 2432)
        cross_attention_dim=3840,            # Z-Image cross attention dimension
        ip_hidden_states_dim=3840,           # IP-Adapter hidden states dimension
        ip_encoder_hidden_states_dim=3840,   # IP encoder hidden states dimension
        head_dim=120,                        # Z-Image head dimension (3840/32)
        timesteps_emb_dim=1280,              # Time embedding dimension (same as SD3)
    ):
        super().__init__()
    ...
    def forward(
        ...
        """
        EXACT COPY of SD3's IPAttnProcessor.forward method.
        """
...
class ZImageBlockIPWrapper:
    """
    EXACT COPY of SD3's JointBlockIPWrapper - just renamed for Z-Image.
    Since Z-Image uses JointTransformerBlock, SD3's wrapper should work perfectly.
    """

    def __init__(
        self,
        original_block,        # This will be a JointTransformerBlock (same as SD3)
        adapter: ZImageIPAttnProcessor,
        ip_options=None,
    ):
    ...
    def block_mixing(self, context, x, context_block, x_block, c):
        """
        EXACT COPY of SD3's block_mixing method.
        """
        print(f"🚨 BLOCK_MIXING CALLED! Context shape: {context.shape if context is not None else None}, X shape: {x.shape if x is not None else None}")
        ...
        if (
            ...
            print(f"🎨 APPLYING IP-ADAPTER! Hidden states shape: {self.ip_options['hidden_states'].shape}, weight: {self.ip_options['weight']}")
            # IP-Adapter
            ...
            if ip_attn is not None:
                print(f"🔥 IP attention applied! Shape: {ip_attn.shape}")
                x_attn = x_attn + ip_attn * self.ip_options["weight"]
            else:
                print(f"⚠️ IP attention was None!")
    ...
    def __call__(self, args, _):
        """
        EXACT COPY of SD3's JointBlockIPWrapper.__call__ method.
        """
        print(f"🚨 WRAPPER CALLED! Args keys: {list(args.keys()) if isinstance(args, dict) else 'not dict'}")
